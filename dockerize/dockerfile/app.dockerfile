...
RUN apk update && apk upgrade && \
    apk add --no-cache bash git vim supervisor freetype libpng libjpeg-turbo freetype-dev libpng-dev libjpeg-turbo-dev libzip-dev libwebp-dev curl build-base nodejs npm tzdata
...
RUN install-php-extensions pdo_mysql bcmath pcntl mbstring sockets zip gd redis-$REDIS_VERSION swoole-$SWOOLE_VERSION

# Clean up unnecessary packages and cache to reduce the image size
RUN apk del --purge freetype-dev libpng-dev libjpeg-turbo-dev libzip-dev libwebp-dev build-base musl-dev g++ && \
